...
import { AiOutlineUser, AiOutlineRobot, AiOutlineReload, AiOutlineClose } from 'react-icons/ai';
          ...
          <div className="fixed bottom-[calc(4rem+1.5rem)] right-0 mr-4 mb-4 bg-white rounded-lg border border-[#e5e7eb] w-[380px] h-[530px] flex flex-col">
            <div className="bg-slate-900 text-white p-4 flex justify-between items-center rounded-t-lg">
              ...
              <div className="flex space-x-2">
                <AiOutlineReload className="cursor-pointer w-6 h-6" onClick={() => setChatHistory([])} />
                <AiOutlineClose className="cursor-pointer w-6 h-6" onClick={closeChatBot} />
              </div>
            </div>
            ...
            <div id="chat-container" className="flex-1 overflow-y-auto p-4">
              {chatHistory.map((message, index) => (
                <div key={index} className={`mb-4 flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex items-start space-x-2 ${message.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`relative p-4 rounded-lg shadow ${message.role === 'user' ? 'bg-slate-400 text-white' : 'bg-gray-200 text-slate-700'}`}>
              ...
              {loading && (
                <div className="flex items-center justify-center text-gray-900 dark:text-gray-100 dark:bg-gray-950">
                  <h1 className="text-sm md:text-xl font-semibold flex">
                    Loading...
                    <svg
                      stroke="currentColor"
                      fill="currentColor"
